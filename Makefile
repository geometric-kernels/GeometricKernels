...
test: ## Run the tests, start with the failing ones and break on first fail.
	@$(UV_RUN) pytest -v -x --ff -rN -Wignore -s --tb=short --durations=0 --cov --cov-report=xml --cov-report=html:coverage_html tests/kernels/test_matern_karhunenloeve_kernel.py::test_eigenvalues_positive
