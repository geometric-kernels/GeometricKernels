...
test:  ## Run the tests, start with the failing ones and break on first fail.
	...
	pytest --nbmake --nbmake-kernel=python3 --durations=0 --nbmake-timeout=1000 --ignore=notebooks/frontends/GPJax.ipynb --ignore=notebooks/frontends/GPflow.ipynb --ignore=notebooks/Stiefel.ipynb --ignore=notebooks/Grassmannian.ipynb notebooks/
