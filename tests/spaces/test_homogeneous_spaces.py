...
class TestCompactLieGroups(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.key, self.manifold = self.manifold(
            n=self.n, m=self.m, key=self.key, average_order=self.average_order
        )
    ...
    def test_feature_map(self) -> None:
        identity = np.eye(self.n, dtype=self.dtype)[..., : self.m].reshape(
            -1, self.n, self.m
        )
